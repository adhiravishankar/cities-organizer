...
import type { Neighborhood } from "@/interfaces/Neighborhood";
...
export class API {
  ...
  async insertNeighborhood(neighborhood: Neighborhood): Promise<KyResponse> {
    ...
    formData.set(
      "middle_school_score",
      neighborhood.MiddleSchoolScore.toString()
    );
    formData.set(
      "elementary_school_score",
      neighborhood.ElementarySchoolScore.toString()
    );
  ...
  async updateNeighborhood(neighborhood: Neighborhood): Promise<KyResponse> {
    ...
    formData.set("max_sqft", neighborhood.MaxSqft.toString());
    formData.set("notes", neighborhood.Notes);
    return ky.post(this.baseURL + "/neighborhoods/" + neighborhood.ID, {
      body: formData,
    });
  }
}
